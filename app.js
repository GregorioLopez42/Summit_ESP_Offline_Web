...
// Registra el Service Worker cuando la aplicación se sirve por HTTPS o localhost.
...
// Si la app ya está instalada y se abrió desde su icono, elimina el banner.
if (isStandalone) {
  ban?.remove();
} else {
  // Android / navegadores Chromium: conserva el instalador nativo.
  window.addEventListener('beforeinstallprompt', event => {
    ...
    if (ban) {
      if (installText) {
        installText.textContent = 'Instalar Summit ESP para uso offline';
      }
  ...
  // iPhone / iPad: Safari requiere Share > Add to Home Screen.
  if (isIOS && ban) {
    ban.innerHTML = `
      <div style="padding:12px;width:100%;line-height:1.55;font-size:14px;position:relative">
        <button id="closeIosGuide" type="button" aria-label="Close"
          style="position:absolute;right:4px;top:0;background:transparent;color:#445;font-size:24px;padding:4px 8px">×</button>

        <div style="font-size:18px;font-weight:bold;color:#c00000;margin:0 34px 12px 0">
          📱 Install Summit ESP on iPhone
        </div>
        ...
        <div style="font-size:13px;color:#555">
          Open Summit ESP from the new Home Screen icon. This guide will not appear inside the installed app.
        </div>
      </div>`;
  ...
  // Ejecutar como archivo local permite cálculos, pero no instalación PWA.
  if (location.protocol === 'file:' && ban) {
    if (installText) {
      installText.textContent =
        'Para instalar como PWA, publique esta carpeta mediante HTTPS.';
    }
  ...
  installBtn?.addEventListener('click', async () => {
    ...
    if (choice.outcome === 'accepted' && ban) {
      ban.hidden = true;
    }
  });
...
// Android: oculta el banner inmediatamente cuando termina la instalación.
